import { Body, Controller, Delete, Get, HttpCode, HttpException, HttpStatus, Param, ParseIntPipe, Post, Put, UseGuards, UsePipes } from "@nestjs/common";
...
import { CreateTenantDto, createTenantSchema } from "src/modules/tenants/dto/tenant-create.dto";
...
import { ApiKeyGuard } from "src/utils/guards/apikey.guard";
...
export class UserController {
     ...
     */
    @Put('/:id/password')
    @UseGuards(new PermissionsGuard(['user-password-change'], { path: 'id', sub: 'sub' }))
    updatePassword(
        @Param('id', ParseIntPipe) id: number,
        @Body(new CustomValidationPipe(updatePasswordSchema)) params: UpdatePasswordDto
    ) {
     ...
     */
    @Put('/:id')
    @UseGuards(new PermissionsGuard(['user-change', '*'], { path: 'id', sub: 'sub' }))
    update(
        @Param('id', ParseIntPipe) id: number,
